...
// Расчет концентрического ripple эффекта для UV координат
...
// Расчет тени для концентрической волны
...
vec4 getZoomedSourceWithRipple(vec2 texCoord, float zoom, float time, sampler2D source) {
    ...
    // Добавляем эффект "ряби на воде" - смещение цвета
