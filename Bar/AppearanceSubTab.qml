...
ColumnLayout {
  ...
  NComboBox {
    Layout.fillWidth: true
    label: I18n.tr("panels.bar.appearance-bar-width-label")
    description: I18n.tr("panels.bar.appearance-bar-width-description")
    visible: Settings.data.bar.position === "top" || Settings.data.bar.position === "bottom"
    model: [
      {
        "key": "full",
        "name": I18n.tr("options.bar.width-full")
      },
      {
        "key": "fixed",
        "name": I18n.tr("options.bar.width-fixed")
      },
      {
        "key": "percent",
        "name": I18n.tr("options.bar.width-percent")
      }
    ]
    currentKey: Settings.data.bar.barWidthMode
    defaultValue: Settings.getDefaultValue("bar.barWidthMode")
    onSelected: key => Settings.data.bar.barWidthMode = key
  }

  NValueSlider {
    Layout.fillWidth: true
    visible: (Settings.data.bar.position === "top" || Settings.data.bar.position === "bottom") && Settings.data.bar.barWidthMode === "percent"
    label: I18n.tr("panels.bar.appearance-bar-percent-width-label")
    description: I18n.tr("panels.bar.appearance-bar-percent-width-description")
    from: 0
    to: 100
    stepSize: 5
    value: Settings.data.bar.barPercentWidth
    defaultValue: Settings.getDefaultValue("bar.barPercentWidth")
    onMoved: value => Settings.data.bar.barPercentWidth = value
    text: Math.floor(Settings.data.bar.barPercentWidth) + "%"
  }

  NValueSlider {
    Layout.fillWidth: true
    visible: (Settings.data.bar.position === "top" || Settings.data.bar.position === "bottom") && Settings.data.bar.barWidthMode === "fixed"
    label: I18n.tr("panels.bar.appearance-bar-fixed-width-label")
    description: I18n.tr("panels.bar.appearance-bar-fixed-width-description")
    from: 600
    to: 1900
    stepSize: 100
    value: Settings.data.bar.barFixedWidth
    defaultValue: Settings.getDefaultValue("bar.barFixedWidth")
    onMoved: value => Settings.data.bar.barFixedWidth = value
    text: Math.floor(Settings.data.bar.barFixedWidth) + "px"
  }
