...
PanelWindow {
  ...
  Item {
    ...
    Item {
      ...
      // Режим ширины: "full" - на весь экран, "fixed" - фиксированная, "percent" - процент от экрана
      readonly property string barWidthMode: "percent"  // "full", "fixed", "percent"
      readonly property real barFixedWidth: 1200     // Фиксированная ширина в пикселях (для mode "fixed")
      readonly property real barPercentWidth: 70     // Процент от ширины экрана (для mode "percent")
      
      // Центрировать ли горизонтальный бар (если ширина меньше экрана)
      ...
      function getBarWidth() {
        if (barIsVertical) {
          return barHeight;  // Для вертикального бара ширина = barHeight
        }
        
        // Для горизонтального бара - применяем кастомную ширину
        ...
        // Применяем режим ширины
        if (barWidthMode === "fixed") {
          return Math.min(barFixedWidth, availableWidth);
        }
        else if (barWidthMode === "percent") {
          var percentWidth = screen.width * (barPercentWidth / 100);
          return Math.min(percentWidth, availableWidth);
        }
        else { // "full" - полная ширина
          return availableWidth;
        }
      }

      // Функция для вычисления позиции X
      function getBarX() {
        // Для вертикального бара слева - прижимаем к левому краю
        if (barPosition === "left") {
          if (isFramed) {
        ...
        // Для вертикального бара справа - прижимаем к правому краю
        if (barPosition === "right") {
          if (isFramed) {
            return screen.width - barHeight - frameThickness;
          }
          return screen.width - barHeight - barMarginH;
        }
        
        // Для горизонтального бара (сверху или снизу) - применяем центрирование
        var barWidth = getBarWidth();
        
        if (isFramed) {
          if (centerBarHorizontally && barWidth < screen.width - frameThickness * 2) {
            return frameThickness + (screen.width - frameThickness * 2 - barWidth) / 2;
          }
          return frameThickness;
        }
        
        if (centerBarHorizontally && barWidth < screen.width - barMarginH * 2) {
          return barMarginH + (screen.width - barMarginH * 2 - barWidth) / 2;
        }
        
        return barMarginH;
      }

      // Функция для вычисления позиции Y
      function getBarY() {
        // Для вертикального бара - на всю высоту (0)
        if (barIsVertical) {
          if (isFramed) {
            return frameThickness;
          }
        ...
        // Для горизонтального бара снизу
        if (barPosition === "bottom") {
          return screen.height - barHeight - barMarginV;
        }
        
        // Для горизонтального бара сверху
        return barMarginV;
      }
      ...
      height: {
        if (!barIsVertical) {
          return barHeight;  // Горизонтальный бар - фиксированная высота
        }
        // Вертикальный бар - на всю высоту экрана
        if (isFramed) {
